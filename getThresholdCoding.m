...
function Latency=getThresholdCoding(lambda,p,N,T)
% lambda=0.5;
% p=0.45;
% N=10000;
% T=8;
...
for i=1:T+1
    ...
    if TransQueue>0 && EstimatedQueue<Threshold
        ...
        EstimatedQueue=EstimatedQueue+p;
        w=w+1;
        if rand>p
        ...
        if EstimatedQueue>=Threshold
            Action(i)=1;
            EstimatedQueue=EstimatedQueue-(1-p);
            if rand>p
                d=d+1;
                if d>=w-0.001
...
for i=T+2:N
    TransQueue=TransQueue+PacketArrive(i);
    EstimatedQueue=RealQueue(i-T)+sum(1-Action(i-T:i-1))*p-sum(Action(i-T:i-1))*(1-p);
    if TransQueue>0 && EstimatedQueue<Threshold
        Action(i)=0; %send an information packet
